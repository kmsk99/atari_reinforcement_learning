...
EXPERIMENT_TAG = "Breakout_PPO_MiniBatch256_Epoch10"
...
MINI_BATCH_SIZE = 256       # 미니배치 크기: 업데이트 내에서 처리할 샘플 크기 (NUM_ENVS * STEPS_PER_UPDATE가 MINI_BATCH_SIZE로 나누어 떨어지면 좋음)
...
def train_ppo(model, optimizer, scaler, all_obs, all_actions, all_old_log_probs, all_returns, all_advantages):
    ...
    for _ in range(PPO_EPOCHS):
        ...
        for i in range(num_minibatches):
            ...
            end = min(start + effective_mini_batch_size, batch_size)
            ...
            with autocast(enabled=scaler.is_enabled()):
                # 현재 정책으로 값 계산
                _, new_log_probs, entropy, values = model.get_action_and_value(mb_obs, mb_actions) # 액션도 전달해야 해당 액션의 로그확률 계산
