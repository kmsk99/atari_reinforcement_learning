...
EXPERIMENT_TAG = "Breakout_PPO_Gamma999_EntDecay"

# 하이퍼파라미터 설정
# 학습 관련 파라미터
GAMMA = 0.999               # 할인계수: 미래 보상의 현재 가치를 계산할 때 사용, 높을수록 미래 보상 중요시
...
MIN_LEARNING_RATE = 1e-6    # 최소 학습률: 이 값 이하로 학습률이 감소하지 않음
PPO_EPOCHS = 4              # PPO 업데이트 에포크: 수집된 데이터로 학습할 반복 횟수
PPO_EPSILON = 0.2           # PPO 클리핑 파라미터: 정책 업데이트 제한, 안정성 향상
ENTROPY_COEF_INITIAL = 0.01         # 초기 엔트로피 계수: 탐험 장려 정도, 높을수록 더 많은 탐험
ENTROPY_COEF_FINAL = 0.001   # 최종 엔트로피 계수: 학습 후반에 적용될 값
...
# PPO 업데이트 함수
def train_ppo(model, optimizer, scaler, observations, actions, log_probs, returns, advantages, current_update=0, total_updates=NUM_UPDATES):
    # 진행 비율 계산 (0~1 사이의 값)
    progress_fraction = min(current_update / total_updates, 1.0) if total_updates > 0 else 0.0
    
    # 엔트로피 계수 선형 감쇠 (초기값에서 최종값으로)
    current_entropy_coef = ENTROPY_COEF_INITIAL * (1 - progress_fraction) + ENTROPY_COEF_FINAL * progress_fraction
...
def main(render, output_dir, checkpoint_dir, filter_dir, layer_dir, gameplay_dir, plot_filename, csv_filename):
    ...
    if render:
        ...
        for update in range(start_update_num, NUM_UPDATES):
            ...
            # PPO 업데이트
            train_ppo(model, optimizer, scaler, observations, actions, log_probs, returns, advantages, current_update=update, total_updates=NUM_UPDATES)
            ...
            if update % 10 == 0:
                ...
                # 현재 엔트로피 계수 계산
                progress_fraction = min(update / NUM_UPDATES, 1.0) if NUM_UPDATES > 0 else 0.0
                current_entropy_coef = ENTROPY_COEF_INITIAL * (1 - progress_fraction) + ENTROPY_COEF_FINAL * progress_fraction
                current_entropy_coef = max(current_entropy_coef, ENTROPY_COEF_FINAL)
                
                print(f"[{current_time}] 업데이트 {update}: 스텝 {total_steps}, 에피소드 {episode_count}, 최근 10개 에피소드 평균 점수: {avg_score:.1f}, 현재 학습률: {current_lr:.6f}, 엔트로피 계수: {current_entropy_coef:.6f}")
