...
def plot_scores(scores, filename, save_csv=True, csv_filename=None):
    # CSV 파일 저장
...
def plot_scores_simple(scores, filename, save_csv=False, csv_filename=None):
    # CSV 파일에서 데이터 읽기
    if csv_filename is not None and os.path.exists(csv_filename):
        df = pd.read_csv(csv_filename)
        scores = df['score'].values
...
def plot_recent_scores(scores, filename, save_csv=False, csv_filename=None):
    # CSV 파일에서 데이터 읽기
    if csv_filename is not None and os.path.exists(csv_filename):
        df = pd.read_csv(csv_filename)
        scores = df['score'].values
...
def plot_all_scores(scores, plot_prefix="training_", csv_filename=None):
    # 모든 그래프 생성
    plot_scores(scores, f"{plot_prefix}scores_full.png", save_csv=True, csv_filename=csv_filename)
    plot_scores_simple(scores, f"{plot_prefix}scores_simple.png", save_csv=False, csv_filename=csv_filename)
    plot_recent_scores(scores, f"{plot_prefix}scores_recent.png", save_csv=False, csv_filename=csv_filename)
...
# 벡터 환경 생성 함수
def make_env(env_id, render=False, max_episode_steps=1000, frame_stack=4, clip_rewards=True):
    """
    AsyncVectorEnv를 위한 환경 생성 함수입니다.
    
    Args:
        env_id: 환경 ID (예: "ALE/Breakout-v5")
        render: 렌더링 여부
        max_episode_steps: 에피소드 최대 길이
        frame_stack: 프레임 스택 수
        clip_rewards: 보상 클리핑 여부
        
    Returns:
        환경 생성 함수
    """
    # 함수 외부 변수를 캡처하지 않도록 모든 인자를 복사
    _env_id = env_id
    _render = render
    _max_episode_steps = max_episode_steps
    _frame_stack = frame_stack
    _clip_rewards = clip_rewards
    
    # 렌더링 모드 미리 결정
    _render_mode = "rgb_array" if _render else None
    
    def _thunk():
        try:
            # 환경 생성
            env = gym.make(_env_id, render_mode=_render_mode)
            
            # 래퍼 적용
            # 그레이스케일 변환 (keep_dim=False면 채널 차원 제거)
            env = gym.wrappers.GrayScaleObservation(env, keep_dim=False)
            
            # 크기 조정 (84x84)
            env = gym.wrappers.ResizeObservation(env, (84, 84))
            
            # 프레임 스택 적용 (여러 프레임을 채널 차원으로 쌓음)
            env = gym.wrappers.FrameStack(env, _frame_stack)
            ...
            if _max_episode_steps is not None:
                env = gym.wrappers.TimeLimit(env, max_episode_steps=_max_episode_steps)
