...
class Qnet(nn.Module):
    def __init__(self, num_actions):
        ...
        # Flatten 후 완전연결 계층에 연결
        self.fc1 = nn.Linear(128 * 128, 512)  # 계산된 출력 크기에 맞게 조정
    ...
    def forward(self, x):
        ...
        # Flatten
        x = x.view(x.size(0), -1)  # 여기에서 x.size(0)는 배치 크기, -1은 나머지 차원을 펼친다는 의미
    ...
    def sample_action(self, obs, epsilon):
        ...
        if obs.dim() == 2:
            # obs = obs.view(-1, 4, 128)  # 배치 차원을 유지하면서 채널과 시퀀스 길이에 맞게 변환
...
def init_frames():
    for _ in range(4):
        frame_queue.append(torch.zeros(128))
