...
# 하이퍼파라미터 설정
GAMMA = 0.99  # 할인계수 (표준값 유지)
LAMBDA = 0.95  # GAE 람다 (표준값 유지)
NUM_ENVS = 32  # 병렬 환경 개수 증가 (16 -> 32)
LEARNING_RATE = 0.001  # 학습률 (0.0003 -> 0.0001로 감소)
PPO_EPOCHS = 4  # PPO 업데이트 에포크 (8 -> 4로 변경)
PPO_EPSILON = 0.2  # PPO 클리핑 파라미터 (고정값 사용)
ENTROPY_COEF = 0.01  # 엔트로피 계수 (유지)
VALUE_COEF = 0.5  # 가치 손실 계수 (1.0 -> 0.5로 변경)
MAX_GRAD_NORM = 0.5  # 그래디언트 클리핑 임계값 (유지)
STEPS_PER_UPDATE = 128  # 업데이트당 스텝 수 (유지)
NUM_MINI_BATCHES = 8  # 미니배치 개수 (유지)
TOTAL_UPDATES = 100_000  # 총 훈련 업데이트 횟수 (약 10만 에피소드)
LOG_INTERVAL = 5  # 로깅 간격 (10 -> 5로 감소)
...
class PPONet(nn.Module):
    def __init__(self, num_actions):
        ...
        # Actor 네트워크 (정책) - 히든 레이어 크기 감소
        self.actor = nn.Sequential(
            nn.Linear(3136, 512),  # 히든 레이어 크기 감소 (1024 -> 512)
            nn.ReLU(),
            nn.Linear(512, num_actions)
        )
        
        # Critic 네트워크 (가치) - 히든 레이어 크기 감소
        ...
        # Actor 마지막 레이어 가중치 스케일링 (작은 값으로 초기화)
        nn.init.orthogonal_(self.actor[-1].weight.data, gain=0.01)
        nn.init.constant_(self.actor[-1].bias.data, 0)
        
        # Critic 마지막 레이어 가중치 스케일링
        nn.init.orthogonal_(self.critic[-1].weight.data, gain=1.0)
        nn.init.constant_(self.critic[-1].bias.data, 0)

    def _init_weights(self, module):
        """모듈의 가중치를 직교 초기화로 설정합니다."""
    ...
    def get_action_and_value(self, obs, action=None):
        if not isinstance(obs, torch.Tensor):
            obs = torch.from_numpy(obs).float()  # /255.0 정규화 제거 (이미 환경 래퍼에서 처리됨)
        obs = obs.to(device)
...
def explained_variance(returns, values):
    ...
    설명된 분산을 계산합니다.
    Returns:
        설명된 분산 값 (1에 가까울수록 좋음)
    """
    var_y = torch.var(returns)
    if var_y == 0:
        return 0.0  # 0으로 나누기 방지
    
    y_pred_error = torch.var(returns - values)
    explained_var = 1 - y_pred_error / var_y
...
def train_ppo(model, optimizer, scaler, observations, actions, log_probs, returns, advantages):
    # 가치 함수 타겟(returns) 정규화 - 학습 안정성 향상
    returns = (returns - returns.mean()) / (returns.std() + 1e-8)
    
    # 전체 데이터 크기
    total_data_size = observations.size(0)
    # 미니배치 개수를 하이퍼파라미터로 설정
    mini_batch_size = total_data_size // NUM_MINI_BATCHES
    ...
    for _ in range(PPO_EPOCHS):
        # 데이터 랜덤 셔플
        indices = torch.randperm(total_data_size)
        
        # 미니배치 단위로 처리
        for start in range(0, total_data_size, mini_batch_size):
            # 배치 인덱스 추출
            end = min(start + mini_batch_size, total_data_size)
            ...
            # 설명된 분산 계산 및 수집
            ev = explained_variance(mb_returns.detach(), values.detach())
            total_explained_var += ev
    
    # 평균 손실 계산 (리턴하여 로깅 가능)
    num_updates = NUM_MINI_BATCHES * PPO_EPOCHS
    avg_actor_loss = total_actor_loss / num_updates
    avg_critic_loss = total_critic_loss / num_updates
    avg_entropy = total_entropy / num_updates
    avg_explained_var = total_explained_var / num_updates
    
    return avg_actor_loss, avg_critic_loss, avg_entropy, avg_explained_var
...
def save_gameplay_gif(model, episode_num, save_path=os.path.join(current_script_path, "gameplay")):
    ...
    # 최대 10000 스텝 또는 게임 종료까지 진행
    for _ in range(10000):
        if done:
            break
            
        # obs는 이미 스택된 프레임 형태이므로 바로 사용
        # numpy 배열을 PyTorch 텐서로 변환 (정규화는 래퍼에서 이미 적용됨)
...
def compute_gae(rewards, values, dones, next_value, gamma=GAMMA, lam=LAMBDA):
    ...
    for t in reversed(range(steps_per_env)):
        # 에피소드 종료 여부에 따른 마스크 (1: 진행 중, 0: 종료)
        mask = 1.0 - dones[t]
        
        # TD 잔차 계산: r_t + gamma * V(s_{t+1}) * mask - V(s_t)
...
def main(render):
    ...
    if checkpoint:
        ...
        if "learning_rate" in checkpoint:  # 저장된 학습률 로드
    ...
    if render:
        ...
        for n_epi in range(start_episode, TOTAL_UPDATES):
            ...
            for step in range(STEPS_PER_UPDATE):
                ...
                # 보상 클리핑 제거 (원래 보상 그대로 사용)
                # clipped_rewards_np = np.sign(rewards_np)
                
                # 완료 여부 확인 (종료 또는 잘림)
                dones_np = np.logical_or(terminated, truncated)
                
                # 에피소드 통계 업데이트 - 원래 보상 사용(클리핑되지 않은)
                                ...
                                save_checkpoint(
                                    {
                                        ...
                                        "episode": n_epi,
                                        "learning_rate": current_lr,  # 현재 학습률 저장
                                    },
                                    episode_count,
                                    saved_scores,
                                )
            ...
            # 학습률 스케줄링 적용
            # 현재 남은 학습 비율 계산 (선형 감소)
            progress = max(1.0 - (n_epi / TOTAL_UPDATES), 0.05)  # 최소 학습률은 초기값의 5%
            current_lr = LEARNING_RATE * progress
            ...
            # PPO 업데이트
            actor_loss, critic_loss, entropy, exp_var = train_ppo(model, optimizer, scaler, observations, actions, log_probs, returns, advantages)
            
            # 터미널 출력에 학습 진행 상황 표시
            if n_epi % LOG_INTERVAL == 0:
                print(f"업데이트 {n_epi}: 스텝 {total_steps}, 에피소드 {episode_count}, 학습률: {current_lr:.6f}, " 
                      f"설명된 분산: {exp_var:.4f}, "
                      f"Actor 손실: {actor_loss:.4f}, Critic 손실: {critic_loss:.4f}, 엔트로피: {entropy:.4f}, "
                      f"최근 10개 에피소드 평균 점수: {np.mean(saved_scores[-10:]):.1f}")
