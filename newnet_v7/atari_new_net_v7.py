...
LEARNING_RATE = 0.0003  # 학습률 (유지)
PPO_EPOCHS = 8  # PPO 업데이트 에포크 (유지)
PPO_EPSILON = 0.2  # PPO 클리핑 파라미터 (고정값 사용)
ENTROPY_COEF = 0.01  # 엔트로피 계수 (유지)
VALUE_COEF = 0.5  # 가치 손실 계수 (유지)
...
class PPONet(nn.Module):
    ...
    def get_action_and_value(self, obs, action=None):
        if not isinstance(obs, torch.Tensor):
            obs = torch.from_numpy(obs).float() / 255.0  # [0, 255] -> [0, 1] 정규화 추가
        obs = obs.to(device)
    ...
    if render:
        ...
        for n_epi in range(start_episode, TOTAL_UPDATES):
            ...
            for step in range(STEPS_PER_UPDATE):
                ...
                # 클리핑된 보상 사용 (학습용)
                rewards.append(torch.FloatTensor(clipped_rewards_np).cpu())
