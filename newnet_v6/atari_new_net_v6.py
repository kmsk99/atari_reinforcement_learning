...
BATCH_SIZE = 128  # 배치 크기 증가 (32 -> 128)
NUM_ENVS = 16  # 병렬 환경 개수 (유지)
LEARNING_RATE = 0.0003  # 학습률 약간 증가 (0.00025 -> 0.0003)
PPO_EPOCHS = 5  # PPO 업데이트 에포크 증가 (4 -> 5)
PPO_EPSILON = 0.1  # PPO 클리핑 파라미터 감소 (0.2 -> 0.1)
ENTROPY_COEF = 0.005  # 엔트로피 계수 감소 (0.01 -> 0.005)
VALUE_COEF = 0.5  # 가치 손실 계수 (유지)
MAX_GRAD_NORM = 0.5  # 그래디언트 클리핑 임계값 (유지)
STEPS_PER_UPDATE = 256  # 업데이트당 스텝 수 증가 (128 -> 256)
NUM_MINI_BATCHES = 8  # 미니배치 개수 증가 (4 -> 8)
...
class PPONet(nn.Module):
    def __init__(self, num_actions):
        ...
        # 입력 크기: 64x9x9
        # 출력 크기: 128x7x7 (채널 수 증가: 64 -> 128)
        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=1)
        
        # 출력 특성 크기: 128 * 7 * 7 = 6272 (이전: 3136)
        ...
        # Actor 네트워크 (정책) - 더 넓은 히든 레이어 사용
        self.actor = nn.Sequential(
            nn.Linear(6272, 1024),  # 히든 레이어 크기 증가 (512 -> 1024)
            nn.ReLU(),
            nn.Linear(1024, num_actions)
        )
        
        # Critic 네트워크 (가치) - 더 넓은 히든 레이어 사용
    ...
    for _ in range(PPO_EPOCHS):
        ...
        for start in range(0, total_data_size, mini_batch_size):
            ...
            with autocast():
                ...
                # Critic 손실 계산 - Huber 손실 사용 (더 안정적인 학습)
...
def main(render):
    ...
    if render:
        ...
        for n_epi in range(start_episode, TOTAL_UPDATES):
            ...
            # PPO 업데이트
            actor_loss, critic_loss, entropy = train_ppo(model, optimizer, scaler, observations, actions, log_probs, returns, advantages)
